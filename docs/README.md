...
Crear la base de datos y las tablas ejecutando el script creacion_suenhatorDB.sql sobre el servidor MySQL.
​

Cargar los datos de ejemplo ejecutando el script de inserción correspondiente (cuando esté definido) en un orden coherente con las relaciones: primero tablas maestras (cliente, invitado, pack, sala, supervisor) y después tablas dependientes (compra, linea_compra, pago, reserva, reserva_invitado, personalizacion).

Configurar la conexión JDBC en la aplicación Java para apuntar a la base de datos suenhator y utilizar los DAOs que trabajan contra este esquema.
